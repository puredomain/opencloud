...
type Config struct {
	...
	// timocloud patch #1 (PATCHES.md): expose reva's storageprovider
	// custom_mimetypes_json hook (upstream issue opencloud-eu/opencloud#3120
	// — the reva-side mechanism exists but nothing in opencloud wires it, so
	// extensions like OOXML Visio resolve to application/octet-stream and
	// never reach the app registry).
	CustomMimeTypesJSON string         `yaml:"custom_mimetypes_json" env:"STORAGE_USERS_CUSTOM_MIMETYPES_JSON" desc:"Path to a JSON file with custom file extension to mime type mappings (e.g. {\"vsdx\": \"application/vnd.ms-visio.drawing.main+xml\"}), registered on top of the built-in mime type table." introductionVersion:"7.2.0-tc"`
...
type FilemetadataCache struct {
	...
	EnableTLS            bool          `yaml:"enable_tls" env:"OC_CACHE_ENABLE_TLS;STORAGE_USERS_FILEMETADATA_CACHE_ENABLE_TLS" desc:"Enable TLS for the connection to file metadata cache." introductionVersion:"7.2.0"`
	TLSInsecure          bool          `yaml:"tls_insecure" env:"OC_INSECURE;OC_CACHE_TLS_INSECURE;STORAGE_USERS_FILEMETADATA_CACHE_TLS_INSECURE" desc:"Whether to verify the server TLS certificates." introductionVersion:"7.2.0"`
...
type IDCache struct {
	...
	EnableTLS            bool          `yaml:"enable_tls" env:"OC_CACHE_ENABLE_TLS;STORAGE_USERS_ID_CACHE_ENABLE_TLS" desc:"Enable TLS for the connection to file metadata cache." introductionVersion:"7.2.0"`
	TLSInsecure          bool          `yaml:"tls_insecure" env:"OC_INSECURE;OC_CACHE_TLS_INSECURE;STORAGE_USERS_ID_CACHE_TLS_INSECURE" desc:"Whether to verify the server TLS certificates." introductionVersion:"7.2.0"`
